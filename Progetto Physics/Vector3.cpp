...
namespace Utils
{
	...
	Quaternion Vector3::toQuaternion() const
	{
		...
		float c1 = cos((this->y * ((static_cast<float>(M_PI)) / 180)) / 2);
		float c2 = cos((this->z * ((static_cast<float>(M_PI)) / 180)) / 2);
		float c3 = cos((this->x * ((static_cast<float>(M_PI)) / 180)) / 2);
		float s1 = sin((this->y * ((static_cast<float>(M_PI)) / 180)) / 2);
		float s2 = sin((this->z * ((static_cast<float>(M_PI)) / 180)) / 2);
		float s3 = sin((this->x * ((static_cast<float>(M_PI)) / 180)) / 2);
